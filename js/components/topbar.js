...
<!-- PERSISTENT MINI TOP BAR -->
<div id="persistent-topbar" 
     class="fixed top-0 left-0 right-0 h-12 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800 shadow-xs z-40 transform -translate-y-full opacity-0 will-change-transform transition-opacity duration-200">
  
  <div class="max-w-md mx-auto h-full flex items-center justify-between px-4">
    
    <!-- Left: Compact Logo & Quick Home Action -->
    <button onclick="if(window.showScannerView) window.showScannerView();" class="flex items-center gap-1.5 active:scale-95 transition-transform text-blue-950 dark:text-blue-400 font-black italic text-xs tracking-tight cursor-pointer">
      <span>CertiFly<span class="text-[6px] font-bold align-super">™</span></span>
      <svg class="h-3.5 w-auto fill-current text-blue-950 dark:text-blue-400" viewBox="0 0 98 72" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Center: Network Connectivity Badge -->
    <div id="topbar-net-status" class="flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase transition-all duration-300 bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400">
      <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
      <span>Online</span>
    </div>

    <!-- Right: Live UTC / Zulu Clock -->
    <div class="flex items-center gap-1 text-[10px] font-mono font-bold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800/80 px-2 py-0.5 rounded-md border border-slate-200/60 dark:border-slate-700">
      <svg class="w-3 h-3 text-blue-600 dark:text-blue-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <span id="topbar-utc-clock">00:00:00Z</span>
    </div>
