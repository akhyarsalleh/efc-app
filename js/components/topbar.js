...
<div id="persistent-topbar" 
  ...
  <div class="max-w-md mx-auto h-full flex items-center justify-between px-3">
    ...
    <!-- Left: Solid Home Button -->
    <button onclick="window.showScannerView()" class="p-2 active:scale-95 transition-transform text-white" aria-label="Home">
    ...
    <!-- Center: Compact Semibold Branding -->
    <div class="flex items-center gap-0.5 font-semibold italic text-sm tracking-tight pointer-events-none">
      <span class="relative">
        CertiFly<span class="align-super text-[5px] font-bold absolute -top-0.5 -right-1.5">™</span>
      </span>
      <svg class="h-4 w-auto fill-current text-white" viewBox="0 0 98 72" xmlns="http://www.w3.org/2000/svg">
    ...
    <!-- Right: Preference Toggles (Square-Rounded Divided Box) -->
    <div class="flex items-center border border-white/20 rounded-lg overflow-hidden h-8">
      <button id="topbar-text-btn" class="p-2 hover:bg-white/10 active:bg-white/20 transition-colors border-r border-white/20 flex items-center justify-center" aria-label="Toggle Text Size">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 21l5.25-11.25L21 21m-9-3h7.5M3 5.25h12M9 3.75v16.5" />
        </svg>
      </button>
      <button id="topbar-theme-btn" class="p-2 hover:bg-white/10 active:bg-white/20 transition-colors flex items-center justify-center" aria-label="Toggle Appearance">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
